...
export default function Models() {
  ...
  return (
    <div className="sectionPadding flex w-full flex-col gap-8 text-center">
      <div className="flex flex-col items-center justify-center text-center">
        <h1 className="text-3xl font-bold capitalize">Choose your transport</h1>
        <p className="text-md m-2 w-full font-medium text-slate-500 lg:w-[500px]">
          Explore our diverse selection of incredible vehicles, select whatever
          perfectly matches your needs for your upcoming adventure or corporate
          journey.
        </p>
      </div>
      <div className="order-3 mt-4 flex flex-col gap-4 lg:order-2 lg:m-8 lg:flex-row lg:gap-12">
        <Image
          className="aspect-video w-full rounded-lg object-cover lg:w-3/5"
        ...
        <div className="w-full lg:w-2/5">
          <table className="w-full table-fixed border-separate border-spacing-2 rounded-lg lg:border-spacing-4">
            <thead>
              <tr className="tableContainer">
                <td className="tableContent">
                  <FaPeace size={iconSize} />
                  Brand
                </td>
            ...
            <tbody className="flex flex-col gap-2 lg:gap-4">
              <tr className="tableContainer">
                <td className="tableContent">
                  <IoLogoModelS size={iconSize}></IoLogoModelS>model
                </td>
                <td className="tableContent">
                  {content.model} {content.year}
                </td>
              </tr>
              <tr className="tableContainer">
                <td className="tableContent">
                  <GiCarDoor size={iconSize}></GiCarDoor>doors/seats
                </td>
                <td className="tableContent">
                  {content.doors}/{content.seats}
                </td>
              </tr>
              <tr className="tableContainer">
                <td className="tableContent">
                  <GiWeight size={iconSize}></GiWeight>load
                </td>
                <td className="tableContent">{content.load}</td>
              </tr>
              <tr className="tableContainer">
                <td className="tableContent">
                  <TbAirConditioning size={iconSize}></TbAirConditioning>ac
                </td>
                <td className="tableContent">{content.ac}</td>
              </tr>
              <tr className="tableContainer">
                <td className="tableContent">
                  <GiGearStick size={iconSize}></GiGearStick>transmission
                </td>
                <td className="tableContent">{content.transmission}</td>
              </tr>
              <tr className="tableContainer">
                <td className="tableContent">
                  <FaGasPump size={iconSize}></FaGasPump>fuel
                </td>
          ...
          <button
            type="button"
            className="callToAction mt-6 uppercase shadow-lg"
          >
            ${content.price} / day
          </button>
        </div>
      </div>
      <div className="order-2 grid grid-cols-2 gap-4 lg:order-3 lg:grid-cols-4 lg:p-8">
        {PricingPlans.map((content) => (
          <div key={content.id}>
            <h1
              onClick={() => {
                toggleActive(content.id);
                handleContent(content.id - 1);
              }}
