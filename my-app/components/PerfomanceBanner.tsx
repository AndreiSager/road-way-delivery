    ...
    return(
        <main className="w-full lg:h-[350px] bg-slate-100 bg-cover bg-bottom py-20 lg:py-0">
            ...
            {BannerContent.map((block) => (
                <div key={block.id} className="h-[180px] lg:h-[220px] lg:w-[250px] aspect-video bg-white rounded-xl p-8 flex flex-row flex-wrap justify-center items-center text-center shadow-2xl hover:scale-105 linear duration-100 transition place-self-center">
                    <Image 
                        src={block.icon}
                        height={150}
                        width={100}
